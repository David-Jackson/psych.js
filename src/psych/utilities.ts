...
export const utilities = {
	humidification: {
		...
		evaporationRate(
			...
			return (outletPoint.properties.W - inletPoint.properties.W) *
				massFlowRate / constants.densityOfWater; // gpm
		},
		...
		maximumSaturation(pt: Point, saturationEfficiency = 1.00) {
			...
			const maxW = (saturationEfficiency *
				(saturationPt.properties.W - pt.properties.W)) +
				pt.properties.W;
	...
	cooling: {
		capacity(inletPoint: Point, outletPoint: Point, volumeInCFM: number) {
			...
			return -(outletPoint.properties.h - inletPoint.properties.h) *
				massFlowRate; // BTU/hr
		},
		...
		flowrate(
			...
			return capacity /
				(specificHeatOfWater * deltaTofCoolingFluid *
					constants.densityOfWater * 60); // gpm
		},
	},
	...
	heating: {
		capacity(inletPoint: Point, outletPoint: Point, volumeInCFM: number) {
			if (inletPoint.properties.W != outletPoint.properties.W) {
				console.warn(
					"Inlet and outlet points don't have some Humidity Ratio, using inlet point's Humidity Ratio",
				);
			}
			const density = 1 / outletPoint.properties.v; // lb/ft^3
			const massFlowRate = density * volumeInCFM * 60; // lb/hr
